...
# urgency words are shared with scoring.py (URGENCY_WORDS) to stay consistent
...
def classify_message(d: Data, msg: dict, media: MediaExtraction) -> Decision:
    ...
    # repetition: how many matched-evidence historical messages were
    # dismissed/muted/reported (ignored) vs opened/replied (engaged).
    ...
    # notification-load / DND dampening: don't strip genuinely urgent items,
    # but borderline notify calls get pulled back to digest if the user is
    # already saturated with notifications today.
