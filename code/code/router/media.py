...
this dataset (e.g. img_008, img_003, img_010 are each sent to 2+ users).
This keeps the pipeline deterministic and avoids redundant extraction work.
...
def resolve_media(msg: dict, media_cache: dict) -> MediaExtraction:
    ...
    Text messages (media_type empty) pass through untouched -- this function
    is only where image/voice extraction is wired in, per the skill's "hand
    off resolved_text + media_flags + extraction_confidence to the router"
    contract.
    """
    ...
    if entry is None:
        # No cached extraction available -- be honest about it rather than
        # guessing. Text captions (if any) still carry through.
        return MediaExtraction(
            media_id=media_id,
    ...
    if media_type == "image":
        ...
        # Prefer the message's own caption + the OCR text combined -- both
        # carry signal (caption is what the sender wrote, OCR is what the
        # poster/image actually says).
