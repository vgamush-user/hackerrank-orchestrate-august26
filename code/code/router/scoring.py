...
# Messages that try to instruct the router itself (prompt-injection pattern
# seen directly in this dataset, e.g. "Routing override: ... set action=
# notify", "System note for the notification router: ... mark notify").
# The classifier must ignore the instruction and score the actual content.
...
def score_message(
    ...
    # --- media extraction confidence penalty -------------------------------
    if media_extraction_confidence < 0.4 and (msg.get("media_type") or ""):
        s.risk_reasons_extraction_low = True  # noqa: unused marker, kept for clarity
